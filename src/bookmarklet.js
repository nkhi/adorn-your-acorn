if (window.location.href == 'https://acorn.utoronto.ca/sws/#/history/academic') {
    ...
    if (v.length > 0) {
        ...
        if (ii != 'NaN' && ii>-1 && ii < 12) {
            ...
            hdr.innerHTML = `<div id=frst><h1 class=&#39;page-title academics&#39;>Nikhil Bhambra</h1><h4>${cols[ii-1]}</h4></div><div id=scnd><h3>Accurate as of: ${date}</h3><h4>This is not an official transcript.</h4></div>`;
...
hdr=document.getElementsByClassName('skipprint')[0];hdr.style.display='flex';hdr.style.justifyContent='space-between';hdr.style.placeItems='center';today=new Date();mnth=today.getMonth();monthNames=["January ","February ","March ","April ","May ","June ","July ","August ","September ","October ","November ","December "];date=monthNames[mnth]+today.getDate()+', '+today.getFullYear();hdr.innerHTML=`<div id=frst><h1 class=&#39;page-title academics&#39;>Nikhil Bhambra</h1><h4>${cols[ii-1]}</h4></div><div id=scnd><h3>Accurate as of: ${date}</h3><h4>This is not an official transcript.</h4></div>`;scnd=document.getElementById('scnd');scnd.style.textAlign='right';scnd.style.marginTop='1rem';regh=document.getElementsByClassName('regHistoryHeader blok pre-elem')[0];regh.style.margin='6px 0 13px';regh.style.paddingLeft='1rem';ps=document.getElementsByTagName('p');ps[0].style.paddingLeft='1rem';ps[1].style.paddingLeft='1rem';hh=document.getElementsByTagName('h1')[0];hh.style.marginBottom='0'}else{alert('Your selection has to be a number between 1 and 10, try again.')}}else{alert('See where it says Complete Academic History next to your name? Click that and try again.')}}else{alert('This bookmarklet only works on Acorn! Please log into Acorn, navigate to Academic History, and try again.')}})()